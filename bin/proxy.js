...
(function() {
  var connections, createServer, exec, fs, inet, inetAton, inetNtoa, localAddr, net, os, path, udpRelay, utils;
  ...
  createServer = function(port, timeout) {
    ...
    server = net.createServer(function(connection) {
      ...
      connection.on("data", function(data) {
        var __localAddr, addrtype, buf, cmd, e, reply, tempBuf;
  ...
  exports.main = function() {
    var PORT, config, configContent, configFromArgs, configPath, item, k, s, timeout, tmpInterfaces, v, value;
    tmpInterfaces = os.networkInterfaces();
    localAddr = (function() {
      var results;
      results = [];
      for (item in tmpInterfaces) {
        value = tmpInterfaces[item];
        if (item.match('MultiVPN') !== null) {
          results.push(value[0].address);
        }
      }
      return results;
    })();
    for (item in tmpInterfaces) {
      value = tmpInterfaces[item];
      if (item.match('MultiVPN') !== null) {
        exec("netsh interface ipv4 add address name=\"" + item + "\" gateway=\"127.0.0.1\" gwmetric=1 store=\"active\"");
      }
    }
